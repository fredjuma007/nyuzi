...
`).replace(/(<\/blockquote>)\n+/g,"$1").replace(/\n+(<blockquote>)/g,"$1").replace(/\n/g,"<br/>")}function Ot(e,i){return e==="upvote"?`<svg class="nyuzi-reaction-icon" viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="2.5" fill="${i?"currentColor":"none"}" stroke-linecap="round" stroke-linejoin="round"><path d="m18 15-6-6-6 6"/></svg>`:e==="like"?`<svg class="nyuzi-reaction-icon nyuzi-like-icon" viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="2" fill="${i?"currentColor":"none"}" stroke-linecap="round" stroke-linejoin="round"><path d="M7 10v12M15 10.5a3 3 0 0 0 3-3V6a3 3 0 0 0-3-3 3 3 0 0 0-3 3v2.5M7 10l5-6v5h7a2 2 0 0 1 2 2v7a2 2 0 0 1-2 2H7"/><path d="M7 10H4a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h3"/></svg>`:`<svg class="nyuzi-reaction-icon nyuzi-heart-icon" viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="${i?"currentColor":"none"}" stroke-linecap="round" stroke-linejoin="round"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"></path></svg>`}function Ut(){return'<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 21 1.9-5.7a8.5 8.5 0 1 1 3.8 3.8z"/></svg>'}function Jt(){return'<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>'}function Yt(){return'<svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg>'}function Kt(){return'<svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg>'}function Vt(e,i){return e.upvotes>0?`${e.upvotes}`:i==="upvote"?"Upvote":"Like"}function Wt(e,i){if(!e)return!1;let t=e.trim().toLowerCase();return i&&t===i.trim().toLowerCase()?!0:t.includes("fred juma")||t.includes("brenda frenjo")||t.includes("sumeiya juma")}function W(e,i){let t=i&&i.length>0?i:["bold","italic","quote","code","link"],a=[];return t.includes("bold")&&a.push(`
  ...
  `}function X(e,i,t){let a=i.filter(d=>d.parentId===e.id);a.sort((d,G)=>new Date(d.createdAt).getTime()-new Date(G.createdAt).getTime());let z=t.activeReplyId===e.id,y=t.editingCommentId===e.id,v=t.confirmDeleteId===e.id,p=t.collapsedComments.has(e.id),x=t.upvotedComments.has(e.id),N=t.myComments.has(e.id),T=e.isAuthor??Wt(e.authorName,t.postAuthor);return`
    <div class="nyuzi-comment" id="comment-${e.id}">
      <div class="nyuzi-avatar">${C(_t(e.authorName))}</div>
  ...
  `}async function tt(e,i,t,a,z,y){let v=y?`&highlight=${encodeURIComponent(y)}`:"",p=`${e}/api/v1/comments?siteId=${encodeURIComponent(i)}&threadUrl=${encodeURIComponent(t)}&page=${a}&limit=${z}${v}`,x=await fetch(p);if(!x.ok)throw new Error(`HTTP ${x.status}`);return x.json()}async function mt(e,i){let t=await fetch(`${e}/api/v1/comments`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)});if(!t.ok){let a=await t.json().catch(()=>({}));throw new Error(a.error||`HTTP ${t.status}`)}return t.json()}async function gt(e,i,t){let a=await fetch(`${e}/api/v1/comments/${encodeURIComponent(i)}/upvote`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:t})});if(!a.ok)throw new Error("Vote action failed");return a.json()}async function yt(e,i,t){let a=await fetch(`${e}/api/v1/comments/${encodeURIComponent(i)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({content:t})});if(!a.ok){let z=await a.json().catch(()=>({}));throw new Error(z.error||`HTTP ${a.status}`)}return a.json()}async function pt(e,i){let t=await fetch(`${e}/api/v1/comments/${encodeURIComponent(i)}`,{method:"DELETE"});if(!t.ok)throw new Error("Delete action failed");return t.json()}async function bt(e,i){let t=await fetch(`${e}/api/v1/threads/react`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)});if(!t.ok)throw new Error("Thread reaction failed");return t.json()}(function(){let e=document.currentScript,i=document.getElementById("nyuzi-comments")||document.querySelector("nyuzi-comments");if(!i){console.warn("[Nyuzi] No container found (#nyuzi-comments or <nyuzi-comments>).");return}let t=i;if(t.__nyuzi_initialized)return;t.__nyuzi_initialized=!0;let a=e?.getAttribute("data-site-id")||t.getAttribute("data-site-id")||document.querySelector("[data-nyuzi-site-id]")?.getAttribute("data-nyuzi-site-id")||"",z=e?.getAttribute("data-api")||t.getAttribute("data-api")||"https://nyuzi-api.fredjuma8.workers.dev",y=e?.getAttribute("data-mock")==="true"||t.getAttribute("data-mock")==="true",v=e?.getAttribute("data-reactions-bar")??t.getAttribute("data-reactions-bar"),p=v===null?!0:v!=="false",x=e?.getAttribute("data-reactions-prompt")||t.getAttribute("data-reactions-prompt")||"How was this discussion?",N=e?.getAttribute("data-reactions-preset")||t.getAttribute("data-reactions-preset")||"general",d=(e?.getAttribute("data-formatting")||t.getAttribute("data-formatting")||"bold,italic,quote,code,link").split(",").map(n=>n.trim().toLowerCase()).filter(Boolean),G=!!(e?.getAttribute("data-accent-color")||t.getAttribute("data-accent-color")),ft=!!(e?.getAttribute("data-theme")||t.getAttribute("data-theme")),ht=!!(e?.getAttribute("data-bg")||t.getAttribute("data-bg")),zt=!!(e?.getAttribute("data-bg-color")||t.getAttribute("data-bg-color")),vt=!!(e?.getAttribute("data-card-bg")||t.getAttribute("data-card-bg")),xt=!!(e?.getAttribute("data-text-color")||t.getAttribute("data-text-color")),wt=!!(e?.getAttribute("data-border-color")||t.getAttribute("data-border-color")),kt=!!(e?.getAttribute("data-radius")||t.getAttribute("data-radius")),$t=!!(e?.getAttribute("data-reaction")||t.getAttribute("data-reaction")),At=v!=null,Et=!!(e?.getAttribute("data-reactions-prompt")||t.getAttribute("data-reactions-prompt")),Ct=!!(e?.getAttribute("data-reactions-preset")||t.getAttribute("data-reactions-preset")),Tt=!!(e?.getAttribute("data-formatting")||t.getAttribute("data-formatting")),b={accent:e?.getAttribute("data-accent-color")||t.getAttribute("data-accent-color")||"#f56220",bg:e?.getAttribute("data-bg-color")||t.getAttribute("data-bg-color")||"",cardBg:e?.getAttribute("data-card-bg")||t.getAttribute("data-card-bg")||"",textColor:e?.getAttribute("data-text-color")||t.getAttribute("data-text-color")||"",textSecondary:e?.getAttribute("data-text-secondary")||t.getAttribute("data-text-secondary")||"",borderColor:e?.getAttribute("data-border-color")||t.getAttribute("data-border-color")||"",inputBg:e?.getAttribute("data-input-bg")||t.getAttribute("data-input-bg")||"",radius:e?.getAttribute("data-radius")||t.getAttribute("data-radius")||"0.75rem",reactionType:e?.getAttribute("data-reaction")||t.getAttribute("data-reaction")||"like",themeMode:e?.getAttribute("data-theme")||t.getAttribute("data-theme")||"auto",bgMode:e?.getAttribute("data-bg")||t.getAttribute("data-bg")||"transparent",showReactionsBar:p,reactionsPrompt:x,reactionsPreset:N,allowedFormatting:d},g=t.shadowRoot||t.attachShadow({mode:"open"});g.innerHTML="",b.themeMode&&t.setAttribute("data-theme",b.themeMode);let P=e?.getAttribute("data-thread-url")||t.getAttribute("data-thread-url")||window.location.href.split("#")[0],et=e?.getAttribute("data-thread-title")||t.getAttribute("data-thread-title")||document.title||"Discussion",Q=e?.getAttribute("data-author-name")||t.getAttribute("data-author-name")||"",f=[],S=0,I=0,Z=1,nt=15,F=!1,q=!1,_=!0,L=null,B=null,H=null,R=null,$=!1,M=null,j=new Set,O=new Set,U=!1,J=`nyuzi_react_${a}_${encodeURIComponent(P)}`,rt=`nyuzi_counts_${a}_${encodeURIComponent(P)}`,E={...y?b.reactionsPreset==="literary"?{coffee:21,book:28,lightbulb:14,heart:19,clap:16}:{fire:18,heart:24,lightbulb:12,laugh:7,clap:15}:{}};try{let n=localStorage.getItem(J);n&&(M=n)}catch{}if(y)try{let n=localStorage.getItem(rt);n&&(E={...E,...JSON.parse(n)})}catch{}let D="nyuzi_reader_ownership",St=900*1e3;function it(n){try{let s=localStorage.getItem(D),c=s?JSON.parse(s):{};c[n]=Date.now()+St,localStorage.setItem(D,JSON.stringify(c))}catch{}}function at(n){try{let s=localStorage.getItem(D);if(s){let c=JSON.parse(s);delete c[n],localStorage.setItem(D,JSON.stringify(c))}}catch{}}function It(){let n=new Set;try{let s=localStorage.getItem(D);if(s){let c=JSON.parse(s),r=Date.now();for(let[h,l]of Object.entries(c))r<l&&n.add(h)}}catch{}return y&&n.add("mock-3"),n}try{let n=sessionStorage.getItem("nyuzi_upvotes");n&&JSON.parse(n).forEach(s=>j.add(s))}catch{}let Y="",K="";try{Y=localStorage.getItem("nyuzi_author_name")||"",K=localStorage.getItem("nyuzi_author_email")||""}catch{}function ot(n,s){n&&(Y=n),s&&(K=s);try{n&&localStorage.setItem("nyuzi_author_name",n),s&&localStorage.setItem("nyuzi_author_email",s)}catch{}}function Mt(){return[{id:"mock-1",parentId:null,authorName:Q||"Fred Juma",authorEmail:"fredjuma8@gmail.com",content:`Welcome to our literary salon! **The Reading Circle** invites your reflections on this essay:

> "A reader lives a thousand lives before he dies. The man who never reads lives only one."

Feel free to share your thoughts, quote your favorite passages, or reply to fellow readers below.`,status:"approved",upvotes:14,createdAt:new Date(Date.now()-36e5*2).toISOString(),isAuthor:!0},{id:"mock-2",parentId:"mock-1",authorName:"Brenda Frenjo",authorEmail:"readingcircle254@gmail.com",content:"The second chapter in particular felt so poignant. The pacing and character progression really resonated with what we discussed during Sunday's book circle session!",status:"approved",upvotes:8,createdAt:new Date(Date.now()-36e5).toISOString(),isAuthor:!0},{id:"mock-3",parentId:null,authorName:"Amina Odhiambo",authorEmail:"amina@example.com",content:"Reading this made me pause and reflect on how we consume stories in the digital age. Check out this related discussion on [Bookish Perspectives](https://readingcircle254.com/blog)!",status:"approved",upvotes:5,createdAt:new Date(Date.now()-18e5).toISOString()}]}function st(){let n=window.location.hash;n&&n.startsWith("#comment-")&&setTimeout(()=>{let s=g.querySelector(n);s&&(s.scrollIntoView({behavior:"smooth",block:"center"}),s.classList.add("nyuzi-highlight"),setTimeout(()=>s.classList.remove("nyuzi-highlight"),3500))},200)}async function Lt(){if(y){f=Mt(),S=f.length,I=f.filter(n=>!n.parentId).length,_=!1,m();return}try{_=!0,Z=1,m();let n=window.location.hash,s=n&&n.startsWith("#comment-")?n.replace("#comment-",""):"",c=await tt(z,a,P,1,nt,s);if(f=c.comments||[],S=c.total||(c.pagination?.totalComments??f.length),I=c.pagination?.totalTopLevel??f.filter(r=>!r.parentId).length,F=c.pagination?.hasMore??!1,y||(E=c.thread?.reactions&&typeof c.thread.reactions=="object"?{...c.thread.reactions}:{}),c.siteSettings&&typeof c.siteSettings=="object"){let r=c.siteSettings;!G&&r.accentColor&&(b.accent=r.accentColor),!ft&&r.themeMode&&(b.themeMode=r.themeMode),!ht&&r.bgMode&&(b.bgMode=r.bgMode),!zt&&r.canvasBg!==void 0&&(b.bg=r.canvasBg),!vt&&r.cardBg!==void 0&&(b.cardBg=r.cardBg),!xt&&r.textColor!==void 0&&(b.textColor=r.textColor),!wt&&r.borderColor!==void 0&&(b.borderColor=r.borderColor),!kt&&r.radiusValue&&(b.radius=r.radiusValue),!$t&&r.reactionType&&(b.reactionType=r.reactionType),!At&&r.showReactionsBar!==void 0&&(b.showReactionsBar=!!r.showReactionsBar),!Et&&r.reactionsPrompt&&(b.reactionsPrompt=r.reactionsPrompt),!Ct&&r.reactionsPreset&&(b.reactionsPreset=r.reactionsPreset),!Tt&&Array.isArray(r.formattingTools)&&(b.allowedFormatting=r.formattingTools)}_=!1,m(),st()}catch(n){console.error("[Nyuzi] Failed to load comments:",n),_=!1,L="Unable to connect to comments server.",m()}}async function Bt(){if(!(q||!F||y))try{q=!0,m();let n=Z+1,s=await tt(z,a,P,n,nt),c=s.comments||[],r=new Set(f.map(h=>h.id));for(let h of c)r.has(h.id)||f.push(h);Z=n,F=s.pagination?.hasMore??!1,I=s.pagination?.totalTopLevel??I,S=s.pagination?.totalComments??S,q=!1,m()}catch(n){console.error("[Nyuzi] Error loading more comments:",n),q=!1,m()}}async function Rt(n){let s=j.has(n),c=s?"unvote":"upvote",r=f.find(h=>h.id===n);s?(j.delete(n),r&&(r.upvotes=Math.max(0,(r.upvotes||1)-1))):(j.add(n),r&&(r.upvotes=(r.upvotes||0)+1));try{sessionStorage.setItem("nyuzi_upvotes",JSON.stringify(Array.from(j)))}catch{}if(m(),!y)try{let h=await gt(z,n,c);r&&typeof h.upvotes=="number"&&(r.upvotes=h.upvotes,m())}catch{s?(j.add(n),r&&(r.upvotes=(r.upvotes||0)+1)):(j.delete(n),r&&(r.upvotes=Math.max(0,(r.upvotes||1)-1))),m()}}async function lt(n,s,c,r,h=null){if(!(!n.trim()||!c.trim()))try{if($=!0,L=null,m(),y){let u={id:`mock-${Date.now()}`,parentId:h,authorName:n.trim(),authorEmail:s,content:c.trim(),status:"approved",upvotes:0,createdAt:new Date().toISOString()};it(u.id),h?f.push(u):(f.unshift(u),I+=1),S+=1,B=null,$=!1,m();return}let l=await mt(z,{siteId:a,threadUrl:P,threadTitle:et,postAuthor:Q,parentId:h,authorName:n,authorEmail:s,content:c,notifyOnReply:r});l.comment&&(it(l.comment.id),h?f.push(l.comment):(f.unshift(l.comment),I+=1),S+=1,B=null),$=!1,m()}catch(l){L=l.message||"Failed to post comment. Please try again.",$=!1,m()}}async function jt(n,s){if(s.trim())try{if($=!0,m(),y){let r=f.find(h=>h.id===n);r&&(r.content=s.trim(),r.isEdited=!0),H=null,$=!1,m();return}await yt(z,n,s.trim());let c=f.find(r=>r.id===n);c&&(c.content=s.trim(),c.isEdited=!0),H=null,$=!1,m()}catch(c){alert(c.message||"Failed to edit comment."),$=!1,m()}}async function Nt(n){try{if($=!0,m(),y){f=f.filter(s=>s.id!==n&&s.parentId!==n),S=f.length,I=f.filter(s=>!s.parentId).length,at(n),R=null,$=!1,m();return}await pt(z,n),f=f.filter(s=>s.id!==n&&s.parentId!==n),S=f.length,I=f.filter(s=>!s.parentId).length,at(n),R=null,$=!1,m()}catch{alert("Failed to delete comment. Please try again."),R=null,$=!1,m()}}function Ht(n,s){let c=n.selectionStart,r=n.selectionEnd,h=n.value,l=h.substring(c,r),u="",o=0;switch(s){case"bold":u=l?`**${l}**`:"**bold text**",o=l?u.length:2;break;case"italic":u=l?`*${l}*`:"*italic text*",o=l?u.length:1;break;case"quote":l?u=l.split(`
`).map(A=>`> ${A}`).join(`
`):u="> quote text",o=u.length;break;case"code":u=l?`\`${l}\``:"`code`",o=l?u.length:1;break;case"link":u=l?`[${l}](https://)`:"[link title](https://example.com)",o=u.length-1;break;default:return}n.value=h.substring(0,c)+u+h.substring(r),n.focus();let w=c+o;n.setSelectionRange(w,w),n.dispatchEvent(new Event("input",{bubbles:!0}))}function m(){let n=f.filter(r=>!r.parentId),s=ut(b),c=It();b.themeMode&&t&&t.setAttribute("data-theme",b.themeMode),g.innerHTML=`
      <style>${s}</style>
      <div class="nyuzi-container">
        ...
        ${_?`<div class="nyuzi-skeleton">
                 ...
                 ${n.map(r=>X(r,f,{postAuthor:Q,reactionType:b.reactionType,upvotedComments:j,activeReplyId:B,editingCommentId:H,confirmDeleteId:R,collapsedComments:O,myComments:c,isSubmitting:$,savedAuthorName:Y,savedAuthorEmail:K,allowedFormatting:b.allowedFormatting})).join("")}
               </div>
